...
export default function AppHeader({
  ...
  return (
    <header className="fixed top-0 z-50 flex h-14 w-full border-b border-outline-variant/70 bg-white/95 shadow-sm backdrop-blur-xl md:h-16 md:shadow-[0_1px_0_rgba(202,211,218,0.72)]">
      <div className="hidden h-full w-72 shrink-0 items-center gap-3 border-r border-outline-variant/70 px-4 md:flex">
        <button
          type="button"
          onClick={onHome}
          className="flex size-10 shrink-0 cursor-pointer items-center justify-center rounded-lg bg-surface-container-low p-1.5 text-on-primary ring-1 ring-outline-variant/70 transition-all hover:bg-white hover:ring-primary/40"
          title="DailyFlash"
          aria-label="Home"
        >
        ...
        <div className="min-w-0 leading-tight">
          <div className="truncate text-[15px] font-extrabold tracking-normal text-on-surface">DailyFlash</div>
          <div className="mt-0.5 truncate text-[10px] font-bold uppercase tracking-[0.16em] text-primary">Centre Point</div>
        </div>
      </div>
      ...
      <div className="flex min-w-0 flex-1 items-center justify-between gap-2 px-3 sm:gap-3 sm:px-4 md:px-5 lg:px-6 xl:px-8">
        ...
        <div className="flex shrink-0 items-center justify-end gap-1.5 sm:gap-2 md:gap-2.5">
          ...
          <div className="hidden items-center rounded-lg border border-outline-variant/70 bg-surface-container-lowest p-1 shadow-sm md:flex">
            <button
              type="button"
              onClick={onRefresh}
              disabled={refreshing}
