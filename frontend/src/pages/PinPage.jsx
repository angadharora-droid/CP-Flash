...
function PinGateCard({
  ...
  return (
    <main className="relative grid min-h-screen place-items-center overflow-hidden bg-[#f6f8fb] px-5 py-10 text-on-surface">
      ...
      <form
        onSubmit={submit}
        className={`relative z-10 w-full max-w-[540px] rounded-[22px] border border-white/80 bg-white/[0.92] px-7 py-9 shadow-[0_30px_90px_rgba(15,23,42,0.14)] backdrop-blur-xl animate-fade-in-up sm:px-12 sm:py-11 ${shake ? 'animate-shake' : ''}`}
      >
        <div className="flex flex-col items-center text-center">
          <div className="flex size-[76px] items-center justify-center rounded-2xl border border-primary/15 bg-white p-2.5 shadow-[0_14px_35px_rgba(23,32,38,0.10)]">
            <img src={cpLogo} alt="Centre Point Hospitality Group logo" className="h-full w-full object-contain" />
          </div>
          <h1 className="mt-5 text-[34px] font-extrabold leading-none tracking-tight text-[#111827] sm:text-[38px]">DailyFlash</h1>
          <p className="mt-2 text-[15px] font-medium text-on-surface-variant sm:text-[17px]">Centre Point Hospitality Group</p>
          <span className="mt-5 h-0.5 w-11 rounded-full bg-primary" aria-hidden />
        </div>

        <div className="mt-9 flex items-start justify-between gap-5">
          <div>
            <p className="text-[11px] font-extrabold uppercase tracking-[0.26em] text-primary/75">Secure Dashboard Access</p>
            <h2 className="mt-3 text-[24px] font-extrabold tracking-tight text-[#111827]">Enter your PIN</h2>
          </div>
        ...
        <label className="relative mt-7 block w-full cursor-text">
          <div className="grid grid-cols-6 gap-3 sm:gap-4">
            {Array.from({ length: 6 }, (_, i) => {
              ...
              return (
                <span key={i} className={`flex aspect-square min-h-12 items-center justify-center rounded-xl border text-[26px] font-bold text-primary transition-all duration-150 sm:min-h-[58px] ${slotBase}`}>
        ...
        <p className="mt-4 text-[13px] font-medium text-on-surface-variant">Enter your 6-digit PIN to access the dashboard</p>
        ...
        <button
          ...
          className="mt-6 flex h-14 w-full items-center justify-center gap-3 rounded-xl bg-gradient-to-r from-[#111827] to-primary px-4 text-[16px] font-extrabold text-white shadow-[0_18px_36px_rgba(163,0,106,0.20)] transition-all hover:brightness-110 active:scale-[0.98] disabled:cursor-not-allowed disabled:bg-none disabled:bg-surface-container-high disabled:text-on-surface-variant disabled:shadow-none"
        >
